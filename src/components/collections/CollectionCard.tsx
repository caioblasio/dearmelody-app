...
type CollectionCardProps = {
  ...
  /** When false, renders a non-navigating card (e.g. form preview). */
  interactive?: boolean
}
...
const cardShellClassName =
  'group flex flex-col overflow-hidden rounded-[20px] border border-warm-border bg-card-bg shadow-sm lg:rounded-[22px]'
...
export function CollectionCard({
  ...
  const body = (
    <>
      ...
      <div className="flex flex-1 flex-col gap-1 p-4 sm:p-5">
        <h2 className="line-clamp-1 font-heading text-lg font-semibold text-ink sm:text-xl">
          {collection.title}
        </h2>
  ...
  if (!interactive) {
    return (
      <article className={cn(cardShellClassName, className)}>{body}</article>
    )
  }
