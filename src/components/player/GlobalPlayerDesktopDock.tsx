...
/** Desktop floating mini player — clicking the track opens its entry page. */
      ...
      <div className="flex max-w-sm items-center gap-3 rounded-full border border-warm-border bg-card-bg/95 py-2 pl-2 pr-3 shadow-lg backdrop-blur-md">
        ...
        >
          {track.imageLocation ? (
            ...
            <div className="album-art-gradient flex size-11 shrink-0 items-center justify-center rounded-full text-sm text-on-primary/90">
              ♪
            </div>
          )}
          <div className="min-w-0 pr-1">
            <p className="truncate text-sm font-semibold text-ink">{track.title}</p>
            <p className="truncate text-xs text-muted">{t('player.nowPlaying')}</p>
          </div>
        </Link>
